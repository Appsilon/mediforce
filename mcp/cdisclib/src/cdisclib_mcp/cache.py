...
When ``CDISCLIB_SNAPSHOT_DIR`` is set, every successful API response is also written to disk
keyed by endpoint + params, so the downstream pipeline can replay runs fully offline against
pinned fixtures (BCs, Dataset Specializations, codelists). This is best-effort: a snapshot
write failure never breaks a tool call (the API result is still returned).
"""
...
_ENV_DIR = "CDISCLIB_SNAPSHOT_DIR"
