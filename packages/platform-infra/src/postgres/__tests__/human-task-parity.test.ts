...
import { InMemoryHumanTaskRepository, buildHumanTask } from '@mediforce/platform-core';
import type {
  HumanTask,
  HumanTaskRepository,
  ProcessInstance,
...
import { PostgresHumanTaskRepository } from '../repositories/human-task-repository';
...
function taskFor(instanceId: string, overrides: Partial<HumanTask> = {}): HumanTask {
...
function contract(
  ...
  describe(`${name} — HumanTaskRepository contract`, () => {
    ...
    it('setDeletedByInstanceIds tombstones tasks and reverses on false', async () => {
      ...
      expect(restored.map((r) => r.id).sort()).toEqual([t1.id, t2.id, t3.id].sort());
    });
