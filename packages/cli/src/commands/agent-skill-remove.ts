...
const HELP = `Usage: mediforce agent skill remove <id> --skill <registryId>:<name> [options]

Remove one or more skill bindings from an agent. Repeats: pass --skill
multiple times. Skills not currently bound are silently ignored. Mirrors
\`git remote remove\` semantics.

Positional:
  <id>                 Agent definition ID

Required flags:
  --skill <ref>        Skill binding to remove, in the form
                       <registryId>:<name>. Repeatable.
...
export async function agentSkillRemoveCommand(input: CommandInput): Promise<number> {
  ...
  try {
    ...
    input.output.stderr(`mediforce agent skill remove: ${String(err)}`);
    input.output.stderr('');
  ...
  try {
    ...
    const filtered = current.filter((ref) => !toRemove.some((r) => sameRef(r, ref)));
    const removed = current.length - filtered.length;
    const result = await mediforce.agents.updateSkills({ id }, { skills: filtered });
    if (jsonMode) {
      printJson(input.output, result);
    } else {
      input.output.stdout(
        `Removed ${String(removed)} skill(s) from agent ${id} (remaining: ${String(filtered.length)})`,
      );
    }
    return 0;
  } catch (err) {
