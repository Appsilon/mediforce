...
import { AUTONOMY_LEVELS, STEP_TYPE_LABELS, FALLBACK_OPTIONS, RUNTIME_OPTIONS } from './constants';
...
export function buildExecutorChangePatch(
  step: WorkflowStep,
  targetExecutor: WorkflowStep['executor'],
  ...
  if (targetExecutor === 'agent') {
    const carriedFromScript = step.script
      ? Object.fromEntries(
          Object.entries(step.script).filter(([k]) =>
            SHARED_CONTAINER_KEYS.includes(k as (typeof SHARED_CONTAINER_KEYS)[number]),
          ),
        )
      : undefined;
    const agent =
      step.agent ??
      (Object.keys(carriedFromScript ?? {}).length > 0 ? (carriedFromScript as WorkflowStep['agent']) : undefined);
    return {
      ...base,
      ...
      databricks: undefined,
      plugin:
        step.plugin === 'script-container' || step.plugin === 'databricks-job'
          ? 'opencode-agent'
          : (step.plugin ?? 'opencode-agent'),
      agent,
    };
  }
  if (targetExecutor === 'script') {
    const carriedFromAgent = step.agent
      ? Object.fromEntries(
          Object.entries(step.agent).filter(([k]) =>
            SHARED_CONTAINER_KEYS.includes(k as (typeof SHARED_CONTAINER_KEYS)[number]),
          ),
        )
      : undefined;
    const script =
      step.script ??
      (Object.keys(carriedFromAgent ?? {}).length > 0 ? (carriedFromAgent as WorkflowStep['script']) : undefined);
    return {
      ...base,
      allowedRoles: undefined,
...
const TIP = {
  ...
  autonomyLevel:
    'How much authority the agent has. L0 = human-only, L2 = human approves output, L4 = fully autonomous.',
  ...
  agentSkill:
    'Skill file name to load at runtime. Skills provide specialised instructions and tools for a specific task.',
  agentSkillsDir:
    "Repo-relative path to the directory containing skill files. Overrides the agent definition's setting.",
  ...
  agentConfidence:
    'Minimum confidence (0–1) the agent must self-report before output is accepted. Below this, the step escalates.',
  agentFallback:
    'What to do if the agent fails or is below the confidence threshold: escalate to human, retry, or skip.',
  ...
  agentDockerfile:
    'Path to a Dockerfile in agent.repo. When set, the container is built from this file instead of agent.image.',
  ...
  scriptDockerfile:
    'Path to a Dockerfile in script.repo. When set, the container is built from this file instead of script.image.',
  ...
  databricksJobId:
    'ID of an existing Databricks job to trigger. Supports ${steps.*} interpolation when given as a string.',
  databricksNotebookParams:
    'notebook_params for run-now as JSON object of string values. Values support ${steps.*} interpolation.',
  databricksJobParameters:
    'job_parameters for run-now (jobs with job-level parameters) as JSON object of string values.',
  ...
  actionKind:
    'Action type: http (outbound API call), reshape (update workflow variables), or email. Fixed at creation.',
  ...
  paramOptions:
    'Fixed list of allowed values, comma-separated. Shown as a dropdown to users instead of a free-text input.',
};
    ...
    <div className="space-y-4" data-testid="step-editor">
          ...
          <FieldGroup>
            <FieldRow label="autonomyLevel" tooltip={TIP.autonomyLevel}>
              <select
                ...
                onChange={(e) =>
                  onChange({ autonomyLevel: (e.target.value || undefined) as WorkflowStep['autonomyLevel'] })
                }
                className={rs}
              >
            ...
            <FieldRow label="agent.confidenceThreshold" tooltip={TIP.agentConfidence}>
              <input
                ...
                onChange={(e) =>
                  updateAgent({ confidenceThreshold: e.target.value ? Number(e.target.value) : undefined })
                }
                className={ri}
              />
            </FieldRow>
            ...
            <FieldRow label="agent.fallbackBehavior" tooltip={TIP.agentFallback}>
              <select
                ...
                onChange={(e) =>
                  updateAgent({
                    fallbackBehavior: (e.target.value || undefined) as WorkflowStep['agent'] extends {
                      fallbackBehavior?: infer F;
                    }
                      ? F
                      : never,
                  })
            ...
            <>
              <FieldRow label="script.runtime" tooltip={TIP.scriptRuntime}>
                <select
                  ...
                  onChange={(e) =>
                    updateScript({
                      runtime: (e.target.value || undefined) as 'javascript' | 'python' | 'r' | 'bash' | undefined,
                    })
                  }
                  className={rs}
                >
            ...
            <>
              <FieldRow label="databricks.jobId" tooltip={TIP.databricksJobId}>
                <input
                  ...
                  onChange={(e) =>
                    updateDatabricks({ jobId: /^\d+$/.test(e.target.value) ? Number(e.target.value) : e.target.value })
                  }
                  className={riMono}
                />
              </FieldRow>
              <FieldRow label="databricks.pollIntervalMs" tooltip={TIP.databricksPollIntervalMs}>
                <input
                  ...
                  onChange={(e) =>
                    updateDatabricks({ pollIntervalMs: e.target.value ? Number(e.target.value) : undefined })
                  }
                  className={riMono}
                />
              </FieldRow>
              <FieldRow label="databricks.timeoutMinutes" tooltip={TIP.databricksTimeoutMinutes}>
                <input
                  ...
                  onChange={(e) =>
                    updateDatabricks({ timeoutMinutes: e.target.value ? Number(e.target.value) : undefined })
                  }
                  className={riMono}
                />
              </FieldRow>
                  ...
                  {Object.entries(step.databricks?.[field] ?? {}).map(([key, val], idx) => (
                    <div key={idx} className="grid grid-cols-[184px_1fr_auto] gap-x-3 py-1 items-center">
                      <input
                        value={key}
                        onChange={(e) =>
                          updateDatabricksParams(field, (params) => {
                      ...
                      <input
                        value={val}
                        onChange={(e) =>
                          updateDatabricksParams(field, (params) => ({ ...params, [key]: e.target.value }))
                        }
                        className={riMono}
                      />
                      <button
        ...
        <FieldGroup>
          <FieldRow label="review.type" tooltip={TIP.reviewType}>
            <select
              ...
              onChange={(e) =>
                updateReview({ type: (e.target.value || undefined) as 'human' | 'agent' | 'none' | undefined })
              }
              className={rs}
            >
              <option value="">Default</option>
            ...
            <>
              <FieldRow label="action.method" tooltip={TIP.actionMethod}>
                <select
                  ...
                  onChange={(e) =>
                    onChange({
                      action: { ...httpAction, config: { ...httpAction.config, method: e.target.value as HttpMethod } },
                    })
                  }
                  className={rs}
                >
              ...
              <FieldRow label="action.url" tooltip={TIP.actionUrl}>
                <input
                  ...
                  onChange={(e) =>
                    onChange({ action: { ...httpAction, config: { ...httpAction.config, url: e.target.value } } })
                  }
                  className={riMono}
                />
              </FieldRow>
              ...
              <div className="border-t border-border/30">
                {Object.entries(httpAction.config.headers ?? {}).map(([hKey, hVal], idx) => (
                  <div
                    key={idx}
                    className="grid grid-cols-[184px_1fr] gap-x-3 px-3 py-1.5 border-b border-border/30 last:border-0 items-center"
                  >
                    <input
                      value={hKey}
                      onChange={(e) => {
                        ...
                        onChange={(e) =>
                          onChange({
                            action: {
                              ...
                              config: {
                                ...httpAction.config,
                                headers: { ...httpAction.config.headers, [hKey]: e.target.value },
                              },
                            },
                          })
                        }
                  ...
                  <button
                    onClick={() =>
                      onChange({
                        action: {
                          ...httpAction,
                          config: { ...httpAction.config, headers: { ...httpAction.config.headers, 'X-Header': '' } },
                        },
                      })
                    }
                    className="text-[11px] text-muted-foreground hover:text-foreground transition-colors"
          ...
          {emailAction && (
            <>
              <FieldRow label="action.to" tooltip={TIP.actionTo}>
                <input
                  value={
                    Array.isArray(emailAction.config.to) ? emailAction.config.to.join(', ') : emailAction.config.to
                  }
                  onChange={(e) => {
                    ...
                    onChange({
                      action: {
                        ...emailAction,
                        config: { ...emailAction.config, to: parts.length === 1 ? parts[0] : parts },
                      },
                    });
                  }}
                  className={ri}
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    onChange({
                      action: {
                        ...emailAction,
                        config: { ...emailAction.config, cc: parts.length > 0 ? parts : undefined },
                      },
                    });
                  }}
                  className={ri}
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    onChange({
                      action: {
                        ...emailAction,
                        config: { ...emailAction.config, bcc: parts.length > 0 ? parts : undefined },
                      },
                    });
                  }}
                  className={ri}
              ...
              <FieldRow label="action.from" tooltip={TIP.actionFrom}>
                <input
                  ...
                  onChange={(e) =>
                    onChange({
                      action: { ...emailAction, config: { ...emailAction.config, from: e.target.value || undefined } },
                    })
                  }
                  className={ri}
                />
                ...
                <input
                  ...
                  onChange={(e) =>
                    onChange({
                      action: {
                        ...emailAction,
                        config: { ...emailAction.config, replyTo: e.target.value || undefined },
                      },
                    })
                  }
                  className={ri}
              ...
              <FieldRow label="action.subject" tooltip={TIP.actionSubject}>
                <input
                  ...
                  onChange={(e) =>
                    onChange({ action: { ...emailAction, config: { ...emailAction.config, subject: e.target.value } } })
                  }
                  className={ri}
                />
              </FieldRow>
              ...
              <FieldRow label="action.body" tooltip={TIP.actionEmailBody} alignStart>
                <textarea
                  ...
                  onChange={(e) =>
                    onChange({ action: { ...emailAction, config: { ...emailAction.config, body: e.target.value } } })
                  }
                  rows={3}
              ...
              <FieldRow label="action.html" tooltip={TIP.actionHtml} alignStart>
                <textarea
                  ...
                  onChange={(e) =>
                    onChange({
                      action: { ...emailAction, config: { ...emailAction.config, html: e.target.value || undefined } },
                    })
                  }
                  rows={3}
              ...
              <FieldRow key={verdictName} label={verdictName} tooltip={TIP.verdictName}>
                <div className="flex items-center gap-1.5">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      onChange({
                        verdicts: { ...step.verdicts, [verdictName]: { ...verdict, target: e.target.value } },
                      })
                    }
                    className={cn(rs, 'flex-1')}
      ...
      {!isTerminal && (
        <Section title="Parameters">
          ...
          <button
            onClick={() =>
              onChange({ params: [...(step.params ?? []), { name: '', type: 'string', required: false }] })
            }
            className="text-[11px] text-muted-foreground hover:text-foreground transition-colors"
          >
            + Add parameter
