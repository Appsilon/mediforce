...
/**
 * One Agent Run's Agent Trajectory (ADR-0023 D8). A run outside the caller's
 * workspaces answers exactly like a run that does not exist.
 */
export async function getAgentTrajectory(
  ...
  const entries = await loadOr404(
    scope.agentTrajectories.list(input.agentRunId),
