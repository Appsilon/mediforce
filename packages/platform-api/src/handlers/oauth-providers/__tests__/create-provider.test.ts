...
import { createOAuthProvider } from '../create-provider';
import { ForbiddenError, HandlerError } from '../../../errors';
import { createTestScope, userCaller } from '../../../repositories/__tests__/create-test-scope';
import { adminRoles, memberRoles, ownerRoles, sampleProviderInput } from './fixtures';
...
describe('createOAuthProvider handler', () => {
  ...
  it('creates a provider for an admin caller and writes audit', async () => {
    ...
    const result = await createOAuthProvider({ namespace: 'alpha', ...sampleProviderInput }, scope);
  ...
  it('throws ForbiddenError for a member-role caller', async () => {
    ...
    await expect(createOAuthProvider({ namespace: 'alpha', ...sampleProviderInput }, scope)).rejects.toBeInstanceOf(
      ForbiddenError,
    );
  ...
  it('throws conflict HandlerError when the id is already taken', async () => {
    ...
    const promise = createOAuthProvider({ namespace: 'alpha', ...sampleProviderInput }, scope);
