...
export interface ResolvedEmailSender {
  ...
  from: string;
  senderName: string;
  provider: 'mailgun' | 'smtp' | 'file';
}

const EMAIL_TO_FILE_ENV = 'MEDIFORCE_EMAIL_TO_FILE';

/**
 * A dev/E2E email sink: instead of sending, append each message as one JSON line
 * to the file named by `MEDIFORCE_EMAIL_TO_FILE`. Lets a local run or a browser
 * test recover the magic-link URL (and any other email) without wiring real
 * SMTP/Mailgun or a catcher like maildev. NEVER set this in production — email
 * silently goes to a file instead of the recipient.
 */
function buildFileSink(filePath: string): SendEmailFn {
  return async (params) => {
    const record = {
...
export function resolveEmailSenderFromEnv(
  ...
  // Dev/E2E file sink takes precedence when set: it makes email observable
  // without a real provider, which is how the magic-link URL is retrieved
  // locally and in browser tests.
  const emailToFile = env[EMAIL_TO_FILE_ENV];
  if (emailToFile !== undefined && emailToFile !== '') {
    return {
      send: buildFileSink(emailToFile),
      from: env.EMAIL_FROM ?? 'dev@localhost',
      senderName: env.EMAIL_SENDER_NAME ?? 'Mediforce',
      provider: 'file',
    };
  }
