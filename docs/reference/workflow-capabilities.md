...
|-----------|-------|-------|
| Custom UI component | `ui.component` + `ui.config` | Resolved by [`task-body-registry.tsx`](../../packages/platform-ui/src/components/tasks/task-body-registry.tsx); registered ids are `file-upload`, `assignment-table`, `table-editor` (each renders its own view). Each component's accepted config is a schema in [`step-ui-config.ts`](../../packages/platform-core/src/schemas/step-ui-config.ts) — `table-editor` takes `columns` (kinds: `static`, `single-select`, `multi-select`, `text`, `avatar`), `assignment-table` takes `assignees`/`priorities`, `file-upload` takes `acceptedTypes`/`minFiles`/`maxFiles`. Unknown ids fall back to the params/verdict/selection views |
...
| Typed output | `agent.outputSchema` (inline JSON Schema; top-level `type: object`, `required` and per-property `type` — `string`, `number`, `integer`, `boolean`, `null`, `array`, `object` — are checked) — `executor: agent` steps only, rejected elsewhere | shown to the agent in its prompt; [`agent-runner.ts`](../../packages/agent-runtime/src/runner/agent-runner.ts) validates `result`, retries once with the error inside the same step timeout, then applies `fallbackBehavior` with reason `output_schema` ([ADR-0023](../adr/0023-step-evaluation.md) D13) |
...
| Few-shot examples | `agent.examples` (up to 20 of `{input, output, note?, caseId?}`; `caseId` cites the Eval Case an example came from) — `executor: agent` steps | rendered as a `## Examples` prompt section after the step's prompt by `buildPrompt` in [`base-container-agent-plugin.ts`](../../packages/agent-runtime/src/plugins/base-container-agent-plugin.ts); part of the Step Fingerprint; cited cases are left out of the Step's Eval Runs ([ADR-0023](../adr/0023-step-evaluation.md) D12) |
...
| Typed artifact out | `outputSchema` (inline JSON Schema; `required` and per-property `type` are checked, as for `agent.outputSchema`) **or** `outputSchemaRef` | `outputSchemaRef: "workflow-definition-authorable"` reuses the WD authorable schema; resolved by `resolveCoworkOutputSchema` in the same file |
