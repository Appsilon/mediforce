...
- **Production (Docker):**
  ...
  ```sh
  node packages/platform-ui/scripts/migrate-postgres.mjs && node packages/platform-ui/server.js
  ```
  The script (
  [`migrate-postgres.mjs`](../../packages/platform-ui/scripts/migrate-postgres.mjs)
...
- **Local dev:** explicit `pnpm db:migrate` (forwards to drizzle-kit CLI
  inside `packages/platform-infra`). The dev server does NOT auto-migrate
  — schema is a deliberate operator step, not hidden in `next dev`. See
...
- **Operator path (pharma on-prem):** same `pnpm db:migrate` invocation
  ...
  process. Or, run the same `migrate-postgres.mjs` script inside their
