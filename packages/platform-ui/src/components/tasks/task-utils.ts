import type { AgentFallbackReason, HumanTask, ProcessInstance, Presentation } from '@mediforce/platform-core';
import { AgentFallbackReasonSchema } from '@mediforce/platform-core';
...
export function getAgentOutput(task: HumanTask): AgentOutputData | null {
  ...
  const parsedFallbackReason = AgentFallbackReasonSchema.safeParse(escalationReason);
  const normalizedEscalation: EscalationReason = parsedFallbackReason.success
    ? parsedFallbackReason.data
...
export type EscalationReason = AgentFallbackReason | 'iterations_limit' | null;
