...
import type { ReviewVerdict, HumanTaskRepository, WorkflowDefinition } from '@mediforce/platform-core';
...
function makeReviewVerdict(verdict: string, comment: string | null = null): ReviewVerdict {
  return {
...
describe('WorkflowEngine', () => {
  ...
  it('createInstance returns ProcessInstance with status created and correct definition reference', async () => {
    const instance = await engine.createInstance('test', 'linear-process', 1, 'user-1', 'manual', { key: 'value' });
  ...
  it('createInstance throws if definition not found', async () => {
    await expect(engine.createInstance('test', 'nonexistent', 99, 'user-1', 'manual')).rejects.toThrow();
  });

  // --- startInstance ---

  it('startInstance transitions created -> running, sets currentStepId to first step', async () => {
    const instance = await engine.createInstance('test', 'linear-process', 1, 'user-1', 'manual', {});
  ...
  it('submitReviewVerdict adds verdict to ReviewTracker, routes via native verdicts', async () => {
    const instance = await engine.createInstance('test', 'review-process', 1, 'user-1', 'manual', {});
    await engine.startInstance(instance.id);
    // Advance from draft to review
    await engine.advanceStep(instance.id, {}, actor);

    const verdict = makeReviewVerdict('approve');
    const result = await engine.submitReviewVerdict(instance.id, 'review', verdict, actor);
  ...
  it('submitReviewVerdict routes a non-first verdict to its declared target (N-way)', async () => {
    ...
    const completedEvent = auditRepo.getAll().find((e) => e.action === 'instance.completed');
    expect(completedEvent).toBeDefined();
    const terminalStepId = (completedEvent!.inputSnapshot as { terminalStepId?: string } | undefined)?.terminalStepId;
  ...
  it('submitReviewVerdict when maxIterations exceeded: pauses instance with reason max_iterations_exceeded', async () => {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      await engine.submitReviewVerdict(instance.id, 'review', makeReviewVerdict('revise'), actor);
    }
    ...
    const maxIterEvent = events.find((e) => e.action === 'review.max_iterations_exceeded');
  ...
  it('resumeInstance on non-paused instance: throws InvalidTransitionError', async () => {
    const instance = await engine.createInstance('test', 'linear-process', 1, 'user-1', 'manual', {});
    await engine.startInstance(instance.id);
    await expect(engine.resumeInstance(instance.id, actor)).rejects.toThrow(InvalidTransitionError);
  });
  ...
  it('full branching flow: when expression routes to correct step based on output', async () => {
    const instance = await engine.createInstance('test', 'branching-process', 1, 'user-1', 'manual', {});
    await engine.startInstance(instance.id);

    // when expression routes to path-b
    const routed = await engine.advanceStep(instance.id, { route: 'b' }, actor);
  ...
  it('full review loop: two revise verdicts then approve -- loops back, then completes', async () => {
    ...
    await engine.submitReviewVerdict(instance.id, 'review', makeReviewVerdict('revise', 'Needs more detail'), actor);
    ...
    await engine.submitReviewVerdict(instance.id, 'review', makeReviewVerdict('revise', 'Still needs work'), actor);
    current = await instanceRepo.getById(instance.id);
    expect(current!.currentStepId).toBe('draft');
    ...
    await engine.submitReviewVerdict(instance.id, 'review', makeReviewVerdict('approve'), actor);
    current = await instanceRepo.getById(instance.id);
  ...
  describe('HumanTask creation', () => {
    it('creates a HumanTask when advancing to a human step', async () => {
      ...
      const instance = await engineWithHumanTasks.createInstance('test', 'linear-process', 1, 'user-1', 'manual', {});
      await engineWithHumanTasks.startInstance(instance.id);
      // Advance from 'start' (automated) -> 'process' (human)
      await engineWithHumanTasks.advanceStep(instance.id, { result: 'done' }, { id: 'user-1', role: 'operator' });
    ...
    it('does not create HumanTask when humanTaskRepository is not injected', async () => {
      ...
      // Should not throw
      await expect(
        engine.advanceStep(instance.id, { result: 'done' }, { id: 'user-1', role: 'operator' }),
    ...
    it('propagates humanTaskRepository.create() failure as advanceStep failure', async () => {
      class FailingHumanTaskRepository extends InMemoryHumanTaskRepository {
        override async create(
          _task: import('@mediforce/platform-core').HumanTask,
        ): Promise<import('@mediforce/platform-core').HumanTask> {
      ...
      const instance = await engineWithFailingRepo.createInstance('test', 'linear-process', 1, 'user-1', 'manual', {});
      await engineWithFailingRepo.startInstance(instance.id);
      // Advancing to human step will trigger create() which throws
      await expect(
        engineWithFailingRepo.advanceStep(instance.id, { result: 'done' }, { id: 'user-1', role: 'operator' }),
    ...
    it('copies the next step verdicts onto the HumanTask as an ordered descriptor array', async () => {
      ...
      await engineWithHumanTasks.startInstance(instance.id);
      await engineWithHumanTasks.advanceStep(instance.id, {}, { id: 'user-1', role: 'operator' });
  ...
  describe('selection review', () => {
    ...
    it('copies selection and options to HumanTask when review step has selection', async () => {
      ...
      const instance = await engineWithSelection.createInstance('test', 'selection-process', 1, 'user-1', 'manual', {});
      ...
      await engineWithSelection.advanceStep(instance.id, optionsPayload, { id: 'user-1', role: 'operator' });
    ...
    it('throws when options count violates selection constraint', async () => {
      ...
      await expect(
        engineWithSelection.advanceStep(instance.id, tooFewOptions, { id: 'user-1', role: 'operator' }),
    ...
    it('copies options to HumanTask even when next step has no selection (e.g. assignment-table)', async () => {
      ...
      const instance = await tableEngine.createInstance('test', 'assignment-process', 1, 'user-1', 'manual', {});
...
describe('WorkflowEngine — WorkflowDefinition (unified schema)', () => {
  ...
  it('createInstance throws when definition not found', async () => {
    await expect(engine.createInstance('test', 'nonexistent', 99, 'user-1', 'manual')).rejects.toThrow(
      "Workflow definition 'nonexistent' version '99' not found",
    );
  });

  it('createInstance uses payload when provided', async () => {
    const instance = await engine.createInstance('test', 'linear-workflow', 1, 'user-1', 'manual', { key: 'value' });
    expect(instance.triggerPayload).toEqual({ key: 'value' });
  });

  it('createInstance defaults payload to empty object when omitted', async () => {
    const instance = await engine.createInstance('test', 'linear-workflow', 1, 'user-1', 'cron');
  ...
  it('advanceStep throws InvalidTransitionError when instance is not running', async () => {
    ...
    await expect(engine.advanceStep(instance.id, {}, actor)).rejects.toThrow(InvalidTransitionError);
  });
  ...
  it('advanceStep creates HumanTask for next human executor step', async () => {
    ...
    await engineWithTasks.advanceStep(instance.id, { result: 'done' }, { id: 'user-1', role: 'operator' });
  ...
  it('[DATA] advanceStep after L2 agent completion routes to next human step and pauses', async () => {
    ...
    const updated = await engine.advanceStep(instance.id, { result: 'agent output' }, actor);
  ...
  it('[DATA] advanceStep after agent completion to terminal step completes the instance', async () => {
    ...
    const updated = await engine.advanceStep(instance.id, { result: 'done' }, actor);
  ...
  it('[DATA] advanceStep after agent completion to another agent step keeps running', async () => {
    ...
    const updated = await engine.advanceStep(instance.id, { result: 'step 1 output' }, actor);
