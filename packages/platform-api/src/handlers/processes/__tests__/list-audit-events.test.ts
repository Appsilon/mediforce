...
describe('listAuditEvents handler', () => {
  ...
  it('returns every event for the instance (api-key)', async () => {
    ...
    const result = await listAuditEvents({ instanceId: 'inst-a' }, scope);
    expect(result.events).toHaveLength(2);
    expect(result.events.map((e) => e.action).sort()).toEqual(['step.completed', 'step.started']);
  });
  ...
  it('throws NotFoundError when the instance does not exist', async () => {
    const scope = createTestScope({ auditRepo, instanceRepo });
    await expect(listAuditEvents({ instanceId: 'inst-missing' }, scope)).rejects.toThrow(NotFoundError);
  });
  ...
  it('throws NotFoundError (not ForbiddenError) for cross-namespace user callers (anti-enumeration)', async () => {
    ...
    await expect(listAuditEvents({ instanceId: 'inst-a' }, scope)).rejects.toThrow(NotFoundError);
  });
  ...
  it('throws NotFoundError when the instance has no namespace', async () => {
    ...
    await expect(listAuditEvents({ instanceId: 'inst-orphan' }, scope)).rejects.toThrow(NotFoundError);
  });
});
