...
export interface AgentOAuthTokenRepository {
  ...
  put(namespace: string, agentId: string, serverName: string, token: AgentOAuthToken): Promise<void>;
  ...
  listByAgent(namespace: string, agentId: string): Promise<Array<AgentOAuthToken & { serverName: string }>>;
}
