...
**Applying migrations at runtime.** `platform-ui`'s
`instrumentation.register()` boot hook calls `applyPostgresMigrations(db)`
([source](../../packages/platform-infra/src/postgres/migrator.ts)) when
`STORAGE_BACKEND=postgres`. Drizzle's `migrate()` reads the journal, runs
each pending file inside a transaction, and records it in
`drizzle.__drizzle_migrations` (schema `drizzle`, table
`__drizzle_migrations` — two underscores, drizzle's own convention).
Subsequent boots are no-ops for already-applied migrations, so the call
is safe on every server start.

**Why app-boot is safe today.** Production is single-container per
`docker-compose.prod.yml` (one `platform-ui`, one `container-worker`).
The `platform-ui` container is the only process that runs migrations.
There is no race because there is no peer.

**Multi-replica future.** When we scale `platform-ui` past one replica,
two replicas booting simultaneously would race for the migration locks
drizzle takes internally — usually safe in practice (the second loses,
sees no pending work, moves on) but not formally guaranteed. The fix is
a Postgres advisory lock around the `migrate()` call
(`pg_advisory_lock(<hash('mediforce-migrations')>)` → migrate → unlock).
Tracked as a follow-up; deferred until we actually run multi-replica.

**Operator path (pharma on-prem).** Customers who want migrations gated
through their own change-management process can disable auto-migrate
(they can leave `STORAGE_BACKEND=postgres` and either pre-apply or
override the boot hook in a downstream build — TBD as we package on-prem
deliveries) and run migrations by hand:

```bash
pnpm --filter @mediforce/platform-infra exec drizzle-kit migrate
```

The drizzle ledger guarantees idempotency, so running this twice or after
the app has already applied the migration is a no-op.
