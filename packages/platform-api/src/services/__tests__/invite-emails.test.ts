...
describe('sendInviteSetupEmail', () => {
  ...
  it('[NO-INVITER] names the workspace without inventing an inviter', async () => {
    const { calls, sendEmail } = captureSend();
    ...
    const sent = calls[0];
    expect(sent.html).toContain('You have been invited to');
    expect(sent.html).toContain('Trial Alpha');
    expect(sent.html).not.toContain('invited you to');
    expect(sent.text).toContain('You have been invited to Trial Alpha on Mediforce.');
  });

  it('[NO-PASSWORD] promises a sign-in, not a password, when password auth is off', async () => {
    const { calls, sendEmail } = captureSend();
    ...
    const sent = calls[0];
    expect(sent.subject).toBe('Sign in to Mediforce');
    expect(sent.html).toContain('Sign in to your account');
    expect(sent.html).toContain('Ada Lovelace');
    expect(sent.html).not.toContain('password');
    expect(sent.text).not.toContain('password');
  });
});
