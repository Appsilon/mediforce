...
- **Tracks:** Phase 4 of [`docs/archive/headless-migration.md`](./headless-migration.md)
- **Relates to:**
...
- **[`docs/archive/headless-migration.md`](./headless-migration.md) Phase 4 section** rewritten to reflect this PRD
...
[`docs/archive/headless-migration.md` § Phase 4](./headless-migration.md)
(CRITICAL LIVE / STANDARD LIVE / NICE LIVE / ONE-SHOT) — verified
per call site, not per hook name (the "live-by-default fallacy" rule
in [`docs/archive/headless-migration.md`](./headless-migration.md)).
...
| PR-final — flip | Delete `lib/firebase.ts` `getFirestore` + emulator. Uninstall `firebase/firestore` from `platform-ui` peer deps. No remaining migration debt — PR4.5 cleared the last writes. Verify `api-boundaries.test.ts` still passes. Update `docs/archive/headless-migration.md` Phase 4 → "done". | Must merge last (after PR4.5). |
...
Mediforce's test taxonomy from [`docs/archive/headless-migration.md`](./headless-migration.md) §"Testing
...
- **Per-resource SSE event stream consolidation.** Future ADR; tracked
  in [`docs/archive/headless-migration.md`](./headless-migration.md) "Captured for after Phase 4."
...
- **Run-executor durability (BullMQ migration).** Captured for later
  in [`docs/archive/headless-migration.md`](./headless-migration.md); independent of UI/API separation.
...
(captured-for-later in [`docs/archive/headless-migration.md`](./headless-migration.md)) will use. Phase 4
...
4. **`useSubcollection` / `useProcessInstance` / `useActiveTaskForInstance` /
   `useActiveCoworkSession`** — phrases used in [`docs/archive/headless-migration.md`](./headless-migration.md)
...
merge, at which point `docs/archive/headless-migration.md` § Phase 4 marks
