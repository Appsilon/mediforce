...
export interface ConnectionRepository {
  ...
  /** Run `fn` while holding a per-Connection lock. The Firestore impl
   *  uses a transaction so concurrent callers serialize on the document;
   *  the in-memory impl uses a Promise-chain mutex per (namespace,id).
   *
   *  `fn` receives the freshly-read Connection (or null if absent) so it
   *  can decide whether work is still needed (e.g. another caller already
   *  refreshed the token) before performing any expensive side effect. */
  runWithLock<T>(
    namespace: string,
    id: string,
