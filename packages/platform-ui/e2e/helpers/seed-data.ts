...
export function buildSeedData(testUserId: string) {
  ...
  const agentRuns: Record<string, Record<string, unknown>> = {
    ...
    // Team dashboard — runs with matching agent definition pluginIds
    'run-risk-detection-1': {
      id: 'run-risk-detection-1',
      processInstanceId: 'proc-running-1',
      stepId: 'risk-scoring',
      ...
      envelope: {
        model: 'openrouter/anthropic/claude-sonnet-4',
        confidence: 0.88,
        confidence_rationale: 'Vendor risk profile well-documented. 2 minor anomalies flagged for review.',
        reasoning_summary: 'Analyzed 8 vendors across 3 regions. 2 minor supply continuity risks identified in APAC suppliers.',
        reasoning_chain: [],
        duration_ms: 2400,
        result: { vendorsAnalyzed: 8, risksFound: 2, severity: 'minor' },
        annotations: [],
      },
      fallbackReason: null,
      startedAt: oneHourAgo,
    ...
    'run-driver-error': {
      id: 'run-driver-error',
      processInstanceId: 'proc-failed-1',
      stepId: 'compliance-check',
      pluginId: 'supply-intelligence/driver-agent',
      autonomyLevel: 'L3',
      status: 'error',
      envelope: null,
      fallbackReason: 'Agent timeout after 30s — upstream API unresponsive',
      startedAt: twoDaysAgo,
      completedAt: twoDaysAgo,
      executorType: 'agent',
    },
    'run-manager-monitoring': {
      id: 'run-manager-monitoring',
      processInstanceId: 'proc-running-1',
      stepId: 'team-monitoring',
      pluginId: 'agent-manager',
      autonomyLevel: 'L4',
      status: 'running',
      envelope: null,
      fallbackReason: null,
  ...
  const agentDefinitions: Record<string, Record<string, unknown>> = {
    ...
    'agent-def-manager': {
      id: 'agent-def-manager',
      pluginId: 'agent-manager',
      name: 'Project Coordinator',
      iconName: 'ClipboardList',
      description: 'Coordinates agent team, monitors quality, tracks progress toward goals, and surfaces blockers.',
      inputDescription: 'Team status, agent runs, and workflow state',
      outputDescription: 'Team health summary, recommendations, and blocker alerts',
      foundationModel: 'anthropic/claude-sonnet-4',
      systemPrompt: '',
      skillFileNames: [],
      createdAt: twoDaysAgo,
