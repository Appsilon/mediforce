...
describe('validateModels', () => {
  ...
  it('detects unknown model and suggests closest match', async () => {
    const result = await validateModels({ modelIds: ['anthropic/claude-haiku-3.5'] }, makeDeps());
  ...
  it('detects bare alias with no close match', async () => {
    const result = await validateModels({ modelIds: ['some-totally-unknown-model'] }, makeDeps());
    expect(result.unknown).toHaveLength(1);
