...
### Changed
- ADR-0005 PR1.1-alt (parallel experiment to PR #494): collapse the two-class platform-API error system into a single `HandlerError` hierarchy on the server (base + per-code subclasses: `NotFoundError`, `ForbiddenError`, `UnauthorizedError`, `ValidationError`, `PreconditionFailedError`, `ConflictError`, `RateLimitedError`). `HandlerError` carries `code: ApiErrorCode` and `details?: unknown`; `route-adapter` collapses to one `instanceof HandlerError` catch arm. Client-side `ApiError` removed; `MediforceClientError` now exposes `code`/`details` as direct fields parsed from the ADR-0005 §1 envelope. ADR-0005 §2 amended in place with rationale for reversing the original subclass rejection.
