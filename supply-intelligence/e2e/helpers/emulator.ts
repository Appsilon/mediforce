...
export async function clearEmulators() {
  ...
  await fetch(
    `${FIRESTORE_EMULATOR}/emulator/v1/projects/${PROJECT_ID}/databases/supply-intelligence/documents`,
    { method: 'DELETE' },
  );
}
...
export async function seedCollection(
  ...
  const basePath = `${FIRESTORE_EMULATOR}/v1/projects/${PROJECT_ID}/databases/supply-intelligence/documents`;
  for (const [docId, docData] of Object.entries(documents)) {
    const res = await fetch(`${basePath}/${collection}?documentId=${docId}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
