...
Executor options grouped by Control Mode (see below). Clicking any enabled option immediately creates the step using the type selected in Section 1. No further wizard steps — agent ID and cowork mode are configured post-creation in the step editor.

---

## Control Mode System (UI only)

The Control Mode (CM0–CM4) is a UI-only classification axis visible in the picker. It is **not related to the L0–L4 `autonomyLevel` schema field** and is never written to storage. It replaces the old 0–4 numeric mode labels.

| Control Mode | Label | Schema mapping | Wizard state |
|---------|-------|---------------|--------------|
| CM0 | No agent | `executor: 'human'` OR `'script'` OR `'action'` | Enabled — three sub-options |
| CM1 | Assist | Not yet implemented | **Disabled — "coming soon"** |
| CM2 | Cowork | `executor: 'cowork'` | Enabled — defaults to `cowork.agent: 'chat'` |
| CM3 | Human review | `executor: 'agent', autonomyLevel: 'L3'` | Enabled |
| CM4 | Autonomous agent | `executor: 'agent', autonomyLevel: 'L4'` | Enabled |

The word "executor" is never shown to users in the picker.

---

## Assist (CM1) — Design Note
...
If a stored step has `autonomyLevel: 'L0'` or `'L1'`, the wizard silently displays it as CM0 (No agent). No warning is shown. Because control mode is never written back to the schema, the stored value is unchanged.

---

## Decisions from Design Grilling (2026-06-22)

1. **Assist (CM1) definition**: Assist means "human leads and executes; AI reviews the result". The original "Ghost" label (L2 = "agent drafts; human approves") is retired from the wizard. Existing L2 steps display as "Assist" for backward compat. Assist in the wizard is disabled pending implementation. The name "Ghost" was changed to "Assist" in June 2026 for clarity.

2. **Control Modes are new, not L-levels**: The CM0–CM4 taxonomy shown in the wizard UI is a new classification axis. It does not map 1:1 to the L0–L4 `autonomyLevel` schema field. Control Modes are a presentational grouping only.
