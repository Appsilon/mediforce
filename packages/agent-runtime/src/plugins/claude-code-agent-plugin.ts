...
export class ClaudeCodeAgentPlugin extends BaseContainerAgentPlugin {
  ...
  getAgentCommand(_promptFilePath: string, options?: SpawnCliOptions): AgentCommandSpec {
    const args: string[] = ['claude', '-p', '--verbose', '--output-format', 'stream-json'];
    ...
    const allowedTools = ['Bash', 'Read', 'Write', 'Edit', 'Glob', 'Grep', ...(this.agentConfig.allowedTools ?? [])];
    ...
    const workflowResolved = isWorkflowAgentContext(this.context) ? this.context.resolvedMcpConfig : undefined;
  ...
  getMockDockerArgs(stepId: string): string[] {
    ...
    return [
      'bash',
      '-c',
      `${copyOutputCmd} && ${copyWorkspaceCmd}; ` + `echo '${mockStreamJson.replace(/'/g, "'\\''")}'`,
    ];
  }
