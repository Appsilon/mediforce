...
> **Superseded — historical.** This is the *pre-implementation* plan, kept for
> its section numbering (code comments cite `PLAN-0002 §1.1`, `§1.4`, `§2.1`,
> `§2.2`, `§3.1`, `§4`, `§7`) and for the reasoning behind the schema shapes.
> It is **not** a description of what shipped. Where it disagrees with the code,
> the code and [ADR-0002](../adr/0002-firebase-auth-to-nextauth.md) — whose
> **Shipped as** notes record every cutover deviation — win. The three biggest
> divergences are flagged inline below: the Credentials provider (§2.1), the
> `ensureUserProfile` bootstrap (§6), and the change-password Server Action
> (§3.2).

Companion to [`0002-firebase-auth-to-nextauth.md`](../adr/0002-firebase-auth-to-nextauth.md).
...
[ADR-0004](../adr/0004-scoped-data-access-authorization.md) is in place.
...
### 1.3 Rename on ADR-0001's `workspace_members` (deferred)

ADR-0001 introduces `workspace_members` with a single `role` column (workspace
governance: `owner | admin | member`). ADR-0002 originally planned to **rename**
it to `membership` to remove the collision with process-domain roles. That
rename is **deferred** — the NextAuth cutover keeps migrations purely additive
and does not touch the live `role` column. The membership-vs-roles distinction
stays a domain concept; the column keeps the name `role` until the domain layer
adopts `membership`:

```sql
-- deferred, not applied in this cutover:
...
> **Not what shipped.** The `Credentials` provider below is impossible under
> `session.strategy: 'database'` — Auth.js throws `UnsupportedStrategy` and
> fails the whole `/api/auth/*` surface at config load. Password sign-in
> shipped as a standalone route, `POST /api/auth/password-login`, doing the
> same bcrypt check and opening the same `auth_sessions` row. The `signIn`
> callback shipped with the allowlist gate only (see §6), and the `session`
> callback carries `user.id` + `user.roles` — no `user_profiles` read, because
> the `deployment_admin` / `current_workspace` columns were never added. See
> `packages/platform-ui/src/auth.ts` and ADR-0002 §4, §4a, §5.
...
- [`packages/platform-ui/src/app/change-password/page.tsx`](../../packages/platform-ui/src/app/change-password/page.tsx) — rewrite to update `auth_users.password_hash` (bcrypt). Only meaningful when `ENABLE_PASSWORD_AUTH=true`. **Not what shipped:** no Server Action — [ADR-0005](../adr/0005-headless-platform-api-ui-separation.md) forbids new ones. It ships as the `setPassword` handler in `packages/platform-api/src/handlers/users/` behind a route adapter, called from the page via the `mediforce` client.
...
[ADR-0004](../adr/0004-scoped-data-access-authorization.md). ADR-0002 fills in
...
> **Not what shipped.** Bootstrap did **not** move into the `signIn` callback.
> It stayed the lazy, idempotent bootstrap in the `getMe` handler
> (`GET /api/users/me`), so handle generation lives in exactly one place and
> runs for every caller regardless of how the session was opened. The `signIn`
> callback does the `ALLOWED_EMAIL_DOMAINS` gate and nothing else. See
> ADR-0002 §4a.
