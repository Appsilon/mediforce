...
import type { CompleteHumanTaskPayload, HumanTask, ProcessInstance } from '@mediforce/platform-core';
...
export async function completeTask(input: CompleteTaskInput, scope: CallerScope): Promise<CompleteTaskOutput> {
...
async function loadTaskContext(scope: CallerScope, taskId: string): Promise<TaskContext> {
  const task = await loadOr404(scope.tasks.getById(taskId), 'Task not found');
  if (scope.caller.kind === 'user' && task.assignedUserId !== null && task.assignedUserId !== scope.caller.uid) {
  ...
  const actorId = scope.caller.kind === 'user' ? actor.actorId : (task.assignedUserId ?? actor.actorId);
...
async function runEngineCompletion(
  ...
  try {
    return await scope.system.engine.completeHumanTask(input.taskId, input.payload, actorId);
  } catch (err) {
...
async function emitAuditEvents(
  ...
  const completionData = (updatedTask.completionData as Record<string, unknown> | null) ?? {};
  const { description, inputSnapshot } = auditFieldsFor(input.payload, input.taskId, resolvedStepId);
...
function auditFieldsFor(payload: CompleteHumanTaskPayload, taskId: string, stepId: string): AuditFields {
