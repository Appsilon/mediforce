...
function NewConfigContent() {
  ...
  const handleSaved = (configName: string, configVersion: string) => {
    router.push(
      `/${handle}/configs/${encodeURIComponent(processName)}/${encodeURIComponent(configName)}/${encodeURIComponent(configVersion)}`,
    );
  };
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <nav className="flex items-center gap-1.5 text-sm text-muted-foreground">
        ...
        <Link
          href={`/${handle}/workflows/${encodeURIComponent(processName)}`}
          className="hover:text-foreground transition-colors"
        >
          {processName}
        </Link>
