import type { InstanceStatus, WorkflowDisplayStatus } from '../schemas/process-instance';
...
/**
 * Derives the UI-facing `WorkflowDisplayStatus` bucket from `{status, pauseReason, error}`.
 *
 * Lives in platform-core (not platform-ui) because the Postgres repository's
 * `displayStatus` filter/aggregation (`process-instance-repository.ts`) needs
 * the exact same bucketing to stay consistent between what the Workflows
 * tab's KPI cards count server-side and what a badge renders client-side.
 * The SQL `CASE` expression there is a hand-ported mirror of this function's
 * branching — any change here must be applied there too, and vice versa.
 */
