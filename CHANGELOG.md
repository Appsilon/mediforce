...
- Dev + test scripts renamed and consolidated so the names match what they do. `pnpm dev:mock` (was `dev:test`) is the zero-setup click-through entry point; `pnpm dev:no-docker` (was `dev:local`) for host `claude`; `pnpm dev:full` (was `dev:ui:queue`) for the production-like queued path; `pnpm worker` / `pnpm redis` / `pnpm emulators` exposed as sidecars without the `dev:` prefix. `pnpm test` now runs everything (`test:unit` + `test:e2e`), `:auth` suffix dropped from E2E variants (every E2E run uses emulators anyway), `pnpm test:e2e` aliased at root with `:api` / `:ui` filters. `bootstrap-dev.py` removed — replaced by `packages/platform-ui/.env.example` + `pnpm emulators` so the path through emulator + seed is explicit instead of bundled.
