...
describe('extractResourceMetadataUrl', () => {
  it('pulls the URL out of an RFC 9728 challenge header', () => {
    ...
    expect(extractResourceMetadataUrl(header)).toBe('https://example.com/.well-known/oauth-protected-resource/mcp');
  });
...
describe('discoverMcpAuthServer', () => {
  ...
  it('follows the challenge hint → resource metadata → AS metadata', async () => {
    ...
    expect(result.authServer.token_endpoint_auth_methods_supported).toEqual(['client_secret_basic', 'none']);
  });
  ...
  it('falls back to .well-known/oauth-protected-resource when no hint is provided', async () => {
    ...
    fetchMock.mockImplementationOnce(
      async () => new Response('{"error":"invalid_token"}', { status: 401, headers: {} }),
    );
    fetchMock.mockImplementationOnce(async (input) => {
      expect(String(input)).toBe('https://mcp.example.com/.well-known/oauth-protected-resource/api');
  ...
  it('throws McpDiscoveryError when resource metadata is malformed', async () => {
    ...
    await expect(discoverMcpAuthServer('https://mcp.example.com/mcp')).rejects.toBeInstanceOf(McpDiscoveryError);
  });
