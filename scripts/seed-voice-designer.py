#!/usr/bin/env python3
"""
Seed the voice-workflow-designer definition into the platform.

Usage:
    python scripts/seed-voice-designer.py [--port PORT]
...
def main():
    ...
    definition_path = Path(__file__).parent.parent / "apps" / "workflow-designer" / "src" / "voice-workflow-designer.wd.json"
        ...
        with urlopen(start_req) as resp:
            ...
            if instance_id:
                ...
                with urlopen(run_req) as run_resp:
                    ...
                    print(f"\nDone! Check Firestore for a voice CoworkSession linked to instance {instance_id}")
    except HTTPError as e:
        error_body = e.read().decode()
        print(f"Error {e.code}: {error_body}")
        sys.exit(1)
