...
describe('workspace seed', () => {
  ...
  it('finds the workspace a step started from and reads it without a worktree', async () => {
    ...
    expect(await listWorkspaceFiles(workspace.repoPath, workspace.seedCommit)).toEqual([
      { path: 'README.md', size: 7 },
      { path: 'data/ae.csv', size: 24 },
    ]);
    expect((await readWorkspaceFile(workspace.repoPath, workspace.seedCommit, 'data/ae.csv'))?.toString()).toBe('AETERM,AETOXGR\nSepsis,5\n');
    expect(await readWorkspaceFile(workspace.repoPath, workspace.seedCommit, 'graded.json')).toBeNull();
  });

  it('applies changes in order and refuses one that does not apply', async () => {
    workspace = gitWorkspace({ 'data/ae.csv': 'AETERM,AETOXGR\nSepsis,5\n', 'logo.bin': 'a\0b' });
    const read = (path: string) => readWorkspaceFile(workspace.repoPath, workspace.seedCommit, path);
  ...
  it('writes the changed workspace as a commit on top of the seed, kept alive by a ref', async () => {
    ...
    expect((await listWorkspaceFiles(workspace.repoPath, commit)).map((file) => file.path)).toEqual(['data/ae.csv', 'data/extra.csv']);
    expect((await readWorkspaceFile(workspace.repoPath, commit, 'data/ae.csv'))?.toString()).toBe('AE_TERM,AETOXGR\nSepsis,5\n');
    // The seed itself is untouched.
    expect((await readWorkspaceFile(workspace.repoPath, workspace.seedCommit, 'data/dm.csv'))?.toString()).toBe('USUBJID\n01-001\n');
  });
});
