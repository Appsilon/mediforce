...
The control-mode rows above answer *who does the work*. That is the right question once you know the shape of the step, and the wrong one when the author simply wants to send an email. So the picker opens on **Simple** — capability-shaped blocks ("Send email", "Call an API", "Route by condition") — with **Full**, the control-mode rows, one click away.
...
- Two preset catalogs now exist and mean different things: `lib/block-presets.ts` (UI-only, the control-mode rows behind the Full tier) and `platform-core/src/blocks/block-presets.ts` (shared with the workflow assistant, behind the Simple tier). The names collide; renaming the UI one to say "control-mode rows" would remove the ambiguity.
