...
describe('Previous run outputs (inputForNextRun)', () => {
  ...
  async function completeRun(
    ...
    const instance = await engine.createInstance('test', defName, version, 'user-1', 'manual', {});
  ...
  describe('first run (no predecessor)', () => {
    it('WD without inputForNextRun: previousRun is undefined', async () => {
      const instance = await engine.createInstance('test', 'plain-process', 1, 'user-1', 'manual', {});
    ...
    it('WD with inputForNextRun, no prior runs: previousRun is empty object', async () => {
      const instance = await engine.createInstance('test', 'sftp-monitor', 1, 'user-1', 'manual', {});
  ...
  describe('second run reads outputs from last completed run', () => {
    it('carries a single output under its exposed name', async () => {
      ...
      const second = await engine.createInstance('test', 'sftp-monitor', 1, 'user-1', 'manual', {});
    ...
    it('carries outputs from different steps', async () => {
      const firstId = await completeRun('multi-carry', 1, [{ cursor: 42 }, { hash: 'abc123', other: 'ignored' }]);

      const second = await engine.createInstance('test', 'multi-carry', 1, 'user-1', 'manual', {});
  ...
  describe('only successful runs count', () => {
    it('skips a failed predecessor and uses the prior completed run', async () => {
      ...
      const failing = await engine.createInstance('test', 'sftp-monitor', 1, 'user-1', 'manual', {});
      ...
      const third = await engine.createInstance('test', 'sftp-monitor', 1, 'user-1', 'manual', {});
    ...
    it('when only failed runs exist, previousRun is empty', async () => {
      ...
      const next = await engine.createInstance('test', 'sftp-monitor', 1, 'user-1', 'manual', {});
    ...
    it('propagates repository errors from createInstance (no silent degradation to {})', async () => {
      ...
      await expect(engine.createInstance('test', 'sftp-monitor', 1, 'user-1', 'manual', {})).rejects.toThrow(repoError);
    });
  });
  ...
  describe('version independence (chain per name)', () => {
    it('carries outputs from previous version of the same workflow name', async () => {
      // v1 completes
      const v1Id = await completeRun('sftp-monitor', 1, [{ cursor: 'v1-value' }]);
      ...
      const v2Instance = await engine.createInstance('test', 'sftp-monitor', 2, 'user-1', 'manual', {});
