...
```bash
node tests/run_tests.mjs        # 57 assertions, all green
```

| Script | Status | What's covered |
|--------|--------|----------------|
| `fetch-candidates` | **tested** | `classifyIssue` (new / pr-open / in-progress ownership / explicit retry / manual re-judge-on-edit / already-classified) + `summariseLabelEvents` attemptCount |
