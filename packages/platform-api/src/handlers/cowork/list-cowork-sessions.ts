 ...
 * `tasks.list({})` GitHub-like default); `namespace` narrows it to the chosen
 * workspaces, as it does there.
 *
...
export async function listCoworkSessions(
  ...
  const base = await scope.coworkSessions.list({ role: input.role, namespaces: input.namespace });
  const statusSet =
