...
or its step executions (Postgres unavailable, network error, permissions,
...
- Resolver query: `getLastCompletedByDefinitionName` selects on
  `definition_name`, `status = 'completed'`, `deleted_at IS NULL`, ordered by
  `updated_at DESC LIMIT 1`
  (`packages/platform-infra/src/postgres/repositories/process-instance-repository.ts`)
- Index: `process_instances_workspace_def_status_idx` on
  `(workspace, definition_name, status, updated_at DESC)`, declared in
  `packages/platform-infra/src/postgres/schema/process-instance.ts` and created
  by the schema migration. Nothing has to be provisioned at deploy time.
- Integration tests:
