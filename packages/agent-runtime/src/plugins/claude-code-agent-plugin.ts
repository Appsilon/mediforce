...
export class ClaudeCodeAgentPlugin extends BaseContainerAgentPlugin {
  ...
  getAgentCommand(_promptFilePath: string, options?: SpawnCliOptions): AgentCommandSpec {
    ...
    // MCP server configuration — generated by prepareOutputDir()
    if (this.agentConfig.mcpServers && this.agentConfig.mcpServers.length > 0) {
      const mcpConfigPath = this.agentConfig.image ? '/output/mcp-config.json' : options?.outputDir ? `${options.outputDir}/mcp-config.json` : '/output/mcp-config.json';
