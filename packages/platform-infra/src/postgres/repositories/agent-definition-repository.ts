...
import { agents } from '../schema/agent-definition.js';

/**
 * Postgres-backed AgentDefinitionRepository (ADR-0001 PR2, PLAN §1.2 agents).
 *
    ...
    const [row] = await this.db
      ...
      .onConflictDoUpdate({
        ...
        set: {
          ...
          skillFileNames: values.skillFileNames,
  ...
  async update(id: string, input: UpdateAgentDefinitionInput): Promise<AgentDefinition> {
    ...
    const [row] = await this.db
      ...
      .set({
        ...
        skillFileNames: [...merged.skillFileNames],
...
function toRow(id: string, input: CreateAgentDefinitionInput) {
  ...
  return {
    ...
    skillFileNames: [...parsed.skillFileNames],
...
function toAgent(row: typeof agents.$inferSelect): AgentDefinition {
  const out: Record<string, unknown> = {
    ...
    skillFileNames: row.skillFileNames,
