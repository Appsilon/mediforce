...
test.describe('Workflow definition namespace isolation — API E2E', () => {
  test('same workflow name registers and resolves independently per namespace', async ({ request }) => {
    ...
    const tenantBFirst = await request.post(`/api/workflow-definitions?namespace=${tenantB}`, {
      headers: authHeaders,
      data: workflowDefinition(name),
    });
    expect(tenantBFirst.status(), await tenantBFirst.text()).toBe(201);
    expect((await tenantBFirst.json()).version).toBe(1);

    const tenantBSecond = await request.post(`/api/workflow-definitions?namespace=${tenantB}`, {
      headers: authHeaders,
      data: workflowDefinition(name),
    });
    ...
    const tenantAFirst = await request.post(`/api/workflow-definitions?namespace=${tenantA}`, {
      headers: authHeaders,
      data: workflowDefinition(name),
    });
    ...
    const tenantAGet = await request.get(`/api/workflow-definitions/${encodeURIComponent(name)}?namespace=${tenantA}`, {
      headers: authHeaders,
    });
    ...
    const tenantBGet = await request.get(`/api/workflow-definitions/${encodeURIComponent(name)}?namespace=${tenantB}`, {
      headers: authHeaders,
    });
    ...
    const tenantAList = await request.get(`/api/workflow-definitions?namespace=${tenantA}`, { headers: authHeaders });
    ...
    expect(matches).toEqual([expect.objectContaining({ namespace: tenantA, latestVersion: 1 })]);
  });
});
