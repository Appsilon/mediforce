...
### Changed
- ADR-0005 Phase 2 PR1: typed error envelope + `tasks/claim` migration. Single `HandlerError` hierarchy in `@mediforce/platform-api` — base class carries `code: ApiErrorCode` + `details?: unknown`, plus per-code subclasses (`NotFoundError`, `ForbiddenError`, `UnauthorizedError`, `ValidationError`, `PreconditionFailedError`, `ConflictError`, `RateLimitedError`). `createRouteAdapter` catches `HandlerError` once and emits the ADR-0005 §1 envelope `{ error: { code, message, details? } }` across every route (Phase 1 + 1.5 reads retroactively typed too). `MediforceClientError` exposes `code`/`details` as direct fields parsed from the envelope. New `POST /api/tasks/:taskId/claim` handler (pure `(input, scope) => { task }`), new `loadOr404` helper, new `mediforce.tasks.claim()` client method. `claimTask` Server Action deleted; `claim-button.tsx` calls the typed client. Audit emission preserved bit-for-bit via `scope.system.audit.append({...})` (handler-resident bridge per ADR-0005 §7).
