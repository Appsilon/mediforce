 ...
 * `tasks.list({})` GitHub-like default); `namespace` narrows it to one
 * workspace, as it does there.
 *
...
export async function listCoworkSessions(
  ...
  const base = await scope.coworkSessions.list({ role: input.role, namespace: input.namespace });
  const statusSet =
