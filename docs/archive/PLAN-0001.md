...
Companion to [`0001-firestore-to-postgres.md`](../adr/0001-firestore-to-postgres.md).
...
[ADR-0004](../adr/0004-scoped-data-access-authorization.md) §7), so the same
...
### 3.0a Hard dependency — the headless client surface (now complete)

Migrating each `onSnapshot` hook off the Firestore client SDK required a
`mediforce.X.Y()` client method backed by a platform-api handler + Zod
contract + route adapter. The full surface now exists — including the reads
that were missing originally (`agent-runs.list`, `agent-runs.getByStepId`,
`processes.list`, `namespaces.*`, `workflow-meta` reads, broader `tasks.list`
across instances and roles).

The realtime swap is **complete**: every UI hook and page-component listener
has moved off `onSnapshot`/the Firestore client SDK to react-query over REST.
platform-ui contains **zero** `firebase/firestore` imports, guarded by
`packages/platform-ui/src/test/integration/api-boundaries.test.ts` (CI fails
on any new `from 'firebase/firestore'` import under `platform-ui/src`).
§10's `grep onSnapshot = 0` and "no `firebase/firestore` in platform-ui/src"
both hold.

### 3.0 Bridge, not destination

Firestore's `onSnapshot` gave us free push-based realtime. Postgres has no
equivalent out of the box. This ADR replaced it with **polling** (landed as
react-query) — known boring tech, ships the migration. Polling gets dedup
(multiple components reading the same hook = 1 request) and refresh-on-focus
(tab-switch refresh) so the "feel" stays acceptable.
...
Three page components subscribed directly (not via hooks). All three have
since moved into hooks and off `onSnapshot` per §3.1:
...
from `index.ts`. **No extraction work remains** — confirmed by the codebase
audit before the Postgres implementations were written.
...
If verification or smoke fails: there is **no** in-place fall back to
Firestore. The migration deletes the Firestore server data layer and Postgres
is the only backend, so rollback means reverting the migration and
re-deploying the previous (pre-migration) image, which reads Firestore again.

1. Re-deploy the previous (pre-migration) container image — it reads Firestore.
2. Restore Firestore from the snapshot if any writes leaked to Postgres after
   the cut.
3. Triage with full `migration_log` + sample diffs. Retry next window — a
   second cutover run is idempotent for natural-key tables.
...
- **Production (Docker):** a dedicated `migrate` compose service (init
  container, build target `migrate` in
  [`packages/platform-ui/Dockerfile`](../../packages/platform-ui/Dockerfile),
  wired up in
  [`docker-compose.prod.yml`](../../docker-compose.prod.yml)) runs:
  ```sh
  pnpm exec drizzle-kit migrate
  ```
  inside `packages/platform-infra`. `platform-ui` waits via
  `depends_on: { migrate: { condition: service_completed_successfully } }`.
  Any migration error fails the service with non-zero exit, blocking the
  app start — fail-stop posture: a half-migrated schema is worse than
  "container failed to start".

- **Local dev:** `pnpm dev` runs `pnpm db:migrate` before
  ...
  [`docs/start/postgres-local-dev.md`](../start/postgres-local-dev.md).
...
- **Operator path (pharma on-prem):** same `pnpm db:migrate` invocation
  ...
  process. Or, run the same `migrate` compose service against their
  Postgres at any point — the ledger guarantees idempotency.

**Why the init-container pattern instead of in-process auto-migrate.**
...
duplicated `postgres` / `drizzle-orm` as `platform-ui` direct deps. A
second attempt (PR #515) chained migration with the app `CMD`
(`node migrate.mjs && node server.js`) but coupled migrate to the
Next.js standalone bundle, which doesn't reliably include `postgres` /
`drizzle-orm`. Staging broke with `ERR_MODULE_NOT_FOUND`; the fix
(PR #570) was a brittle `outputFileTracingIncludes` glob list
hard-coded to pnpm's symlink layout. The init-container approach drops
both: the app standalone bundle never sees the DB driver, migrate has
its own image with the full workspace + devDeps (drizzle-kit) available,
and the pattern matches the canonical Rails/Django/Prisma model
(migrate as a separate process step, not a side-effect of app boot).
...
- `pnpm dev` runs the app against the dockerised Postgres (no Redis; `dev:queue` adds Redis).
...
1. `grep -r "onSnapshot" packages/` returns zero matches.
   - Achieved: every UI hook and page listener has moved off `onSnapshot`
     to react-query over REST. See §3.0a.
2. `grep -r "firebase/firestore\|firebase-admin/firestore" packages/` returns
   zero matches.
   - Achieved for `platform-ui/src` and guarded by
     `packages/platform-ui/src/test/integration/api-boundaries.test.ts`,
     which fails CI on any new `from 'firebase/firestore'` import there.
     (Firebase **Auth** + the server-side `users` collection read remain on
     Firebase under ADR-0002 scope — see §11.)
