...
Browse LLM models synced from OpenRouter — search by name, filter by provider or capabilities (tools, vision), sort by context length or popularity. The registry provides model metadata used when configuring workflow steps.

![model-registry](model-registry.gif)
