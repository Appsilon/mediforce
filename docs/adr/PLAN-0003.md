...
create table task_attachments (
  ...
  -- `text` Firebase uid, NOT a FK to auth_users: ADR-0003 lands before
  -- ADR-0002, so auth_users does not exist yet. Consistent with the rest of
  -- the user graph today (audit_events.actor_id, workspace_members.uid are
  -- text uids). ADR-0002's staging remap rewrites this column when fresh
  -- uuids land.
...
**No Server Actions** (ADR-0005 / AGENTS.md §8). Every write below is a
headless handler in `packages/platform-api/src/handlers/` + a Zod contract +
a route adapter in `packages/platform-ui/src/app/api/`, called from the UI via
`mediforce.X.Y()`.

- [`packages/platform-ui/src/app/(app)/[handle]/agents/new/page.tsx`](../../packages/platform-ui/src/app/(app)/[handle]/agents/new/page.tsx) — `uploadBytes` to `agentSkills/...` → `mediforce.agents.uploadSkill(...)` (multipart route adapter → handler `INSERT`s into `agent_definition_skills`).
...
- [`packages/platform-ui/src/components/tasks/task-detail.tsx`](../../packages/platform-ui/src/components/tasks/task-detail.tsx) — `uploadBytesResumable` + `getDownloadURL` → POST to the new headless route `/api/tasks/{id}/attachments` (handler writes `task_attachments` + `task_attachment_blobs`).
...
### 2.5 Storage-only migration script (standalone, one-off)

ADR-0001's Firestore → Postgres cutover **already shipped** (#534), so this is
a **separate** storage-only ingestion script (run once against staging), not an
addition to a script that already ran. Two tasks:
...
Simplify: the form submits files to the headless skill-upload route
(`mediforce.agents.uploadSkill(...)`), which writes directly to
`agent_definition_skills`. No client SDK upload, no Server Action. Progress
indication via the mutation's pending state.
