...
| `07-trigger-varieties.wd.json` | The unified `triggerInput` contract — one set of fields read as `${triggerPayload.*}` whichever trigger fires, plus the `${triggerContext.*}` transport hatch (ADR-0012). Triggers themselves are table resources, not declared on the definition (ADR-0011) |
