...
export async function POST(
  ...
  try {
    ...
    // This prevents the undici/proxy headers timeout from killing the handler
    // on long agent steps. The loop continues in the background and updates
    // the instance in Firestore; clients poll for state.
    after(async () => {
      let stepsExecuted = 0;
      try {
