...
describe('estimateEvalRun', () => {
  it('uses the step\'s mean production cost when it has history', async () => {
    ...
    const estimate = await estimateEvalRun(fixture.scope(), STEP, workflowStep, [], 10);
    expect(estimate).toEqual({ perTrialUsd: 0.3, totalUsd: 3, basis: 'history', sampleSize: 2 });
  });
  ...
  it('falls back to the agent model\'s price for a nominal turn, plus one call per judge', async () => {
    ...
    const estimate = await estimateEvalRun(scope, STEP, workflowStep, [judge], 4);
    // Agent: 50k × 3e-6 + 5k × 15e-6 = 0.225; judge: 4k × 1e-6 + 500 × 5e-6 = 0.0065.
    expect(estimate).toEqual({ perTrialUsd: 0.2315, totalUsd: 0.926, basis: 'model_pricing', sampleSize: 0 });
  });

  it('has no estimate when neither history nor a price is known', async () => {
    const fixture = await evaluationFixture();
    const estimate = await estimateEvalRun(withPrices(fixture.scope(), {}), STEP, workflowStep, [], 4);
    expect(estimate).toEqual({ perTrialUsd: null, totalUsd: null, basis: 'unknown', sampleSize: 0 });
  });
});
